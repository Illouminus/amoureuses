			...
			{menu.map(item => category._id === item.category && (
				<div className={styles.link} key={item._id}>
					...
					<div className={styles.spanGroup}>
						{status === "authenticated" && <span className={classnames(styles.contour__delete, activeDelete === item.id && styles.active)} onClick={() => {
							deleteHendler(item._id)
							deleteItem(item._id)
						}}>
							<span></span><p className={styles.contour__delete__text}>Delete</p>
						</span>}
						{status === "authenticated" && <span className={styles.editSpan} onClick={() => {
							findId(item._id)
							setOpen(true)
						}}>
							<svg
